...
function toInlineString(val: any): string {
  ...
  if (typeof val === "object") {
    if ("code" in val && typeof (val as any).code === "string") return (val as any).code
...
function pickOneSolve(
  solveData: any,
  opts: { solveId?: number | string; currentUserId?: string; fallbackType?: ProblemVariant["problemType"] }
) {
  ...
  // 1) solveId 매칭
  if (solveId != null) {
    const one = fixed.find((x) => String(x.solve_id ?? x.id) === String(solveId))
    if (one) return one
  }
  // 2) 내 제출 최신
  if (currentUserId) {
...
function hasAnswerPayload(s: any, type?: ProblemVariant["problemType"]) {
  ...
  switch (t) {
    ...
    case "객관식":
      return s?.selected_options != null || s?.selected_index != null || s?.selected_indices != null
    case "단답형":
      return (Array.isArray(s?.answers) && s?.answers.length > 0) || !!s?.submitted_text || !!s?.answer_text
    case "주관식":
      return !!(s?.written_text || s?.submitted_text || s?.text || s?.answer || s?.essay || s?.content)
    default:
      return false
  }
}
  ...
  switch (type) {
    ...
    case "디버깅": {
      ...
      if (!codeCandidate && raw?.codes) {
        const c = Array.isArray(raw.codes) ? raw.codes.find((x: any) => typeof x?.code === "string") : raw.codes
        if (c?.code) codeCandidate = c.code
        if (!langCandidate && c?.language) langCandidate = c.language
      }

      // 깊은 곳 탐색(최대 2단)
      if (!codeCandidate && raw?.answer && typeof raw.answer === "object") codeCandidate = raw.answer.code
      if (!langCandidate && raw?.answer && typeof raw.answer === "object") langCandidate = raw.answer.language
    ...
    case "객관식": {
      let arr =
        raw?.selected_options ?? raw?.selectedOptions ?? raw?.selected_indices ?? raw?.selected_index
      if (arr == null) {
        if (Array.isArray(raw?.answer)) arr = raw?.answer
        else if (typeof raw?.answer === "number") arr = [raw?.answer]
      }
      n.selectedOptions = Array.isArray(arr) ? arr.map((x) => Number(x)) : arr != null ? [Number(arr)] : []
      break
    }

    case "단답형": {
      let a =
        raw?.answers ?? raw?.answer_text ?? raw?.submitted_text ?? raw?.submittedText ?? raw?.text
      if (a == null && typeof raw?.answer === "string") a = [raw?.answer]
      if (a == null && typeof raw?.answer === "object" && raw?.answer?.text) a = [raw?.answer?.text]
...
export default function AnswerRenderer({
  ...
  const fallbackType = (problem as any)?.problemType as ProblemVariant["problemType"]
  ...
  // 3) 렌더용으로 정규화
  const s = useMemo(() => normalizeSolve(hydrated ?? picked, fallbackType), [hydrated, picked, fallbackType])
    ...
    case "디버깅": {
      return (
        <div className="p-4 bg-white rounded-lg border">
          ...
          {debug && (
            <DebugBox title="코딩/디버깅 raw" obj={s.raw} />
          )}
        </div>
      )
    }
              ...
              {selected.map((idx) => {
                ...
                return (
                  <li key={idx} className="flex items-start gap-2">
                    <span className={`px-2 py-0.5 text-xs rounded ${isCorrect ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}>
                      {isCorrect ? "정답" : "오답"}
                    </span>
              ...
              {myAnswers.map((ans: string, i: number) => {
                ...
                return (
                  <li key={i} className="flex items-start gap-2">
                    <span className={`px-2 py-0.5 text-xs rounded ${ok ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}>
                      {ok ? "정답" : "오답"}
                    </span>
...
function DebugBox({ title, obj }: { title: string; obj: any }) {
  return (
    <details className="mt-4">
      <summary className="cursor-pointer text-sm text-gray-500">{title}</summary>
