...
export default function NewRegisteredProblem() {//외부에서 사용가능하게끔
	...
	const memoizedLoadDraft = useCallback(() => {
		loadDraft();
	}, [loadDraft]);

	const memoizedSaveDraft = useCallback(() => {
		saveDraft();
	}, [saveDraft]);
	...
	// 상태 변경 시 드래프트 저장 (의존성 배열을 구체적으로 지정)
	useEffect(() => {
		memoizedSaveDraft();
	}, [title, description, difficulty, ratingMode, tags, conditions, referenceCodes, testCases, memoizedSaveDraft]);

	// Markdown 미리보기 생성 함수
	const generateMarkdownPreview = useCallback((markdown: string) => {
