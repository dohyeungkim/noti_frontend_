...
export default function ManageGroup() {
  ...
  const acceptAllRequests = useCallback(async () => {
    if (groupInvMembers.length === 0) {
      alert("신청한 멤버가 없습니다.");
      setShowModalAccAll(false);
      return;
    }
    try {
      setIsBulkAccepting(true);
      // 한 명이라도 실패해도 나머지는 진행되도록 allSettled 사용
      const results = await Promise.allSettled(
        groupInvMembers.map((m) =>
          group_member_api.group_member_req_response(
            Number(groupId),
            m.user_id,
            true
          )
      ...
      if (rejected.length === 0) {
        alert("전체 수락이 완료되었습니다.");
      } else {
        alert(`일부 수락에 실패했습니다. (실패: ${rejected.length}명)`);
      }
      ...
      console.error("전체 수락 처리 중 오류", e);
      alert("전체 수락 처리 중 오류가 발생했습니다.");
    } finally {
      setIsBulkAccepting(false);
      setShowModalAccAll(false);
    }
  }, [groupInvMembers, groupId, fetchPrivateGroupMemberReq, fetchGroupMember]);
    ...
    <div className="w-full flex flex-col bg-transparent p-5 overflow-auto">
                  ...
                  {groupMembers.length > 0 ? (
                    groupMembers.map((member) => (
                      <tr key={member.user_id}>
                        ...
                        <td className="border border-[#6c6c6c] p-2 text-left">
                          {new Date(
                            member.timestamp_requested
                          ).toLocaleDateString()}
                        </td>
                        <td className="border border-[#6c6c6c] p-2 text-left">
                          {new Date(
                            member.timestamp_approved
                          ).toLocaleDateString()}
                        </td>
                        <td className="border border-[#6c6c6c] p-2 text-left">
                          <button
      ...
      >
        <div className="flex justify-between items-center">
          ...
          {/* ✅ 버튼 묶음: 전체 수락 + 신청 멤버 조회 */}
          <div className="flex items-center gap-2">
            <button
              className="bg-[#7fb18c] w-[110px] h-[33px] rounded-[10px] disabled:opacity-60"
              onClick={() => setShowModalAccAll(true)}
        ...
        {/* 모달 - 전체 수락 확인 */}
        <Modal show={showModalAccAll}>
          <div className="fixed top-0 left-0 w-full h-full bg-black/50 flex justify-center items-center z-[1000]">
            <div className="bg-white p-[30px] rounded-[18px] text-center min-w-[300px]">
              <h3 className="text-center">
                대기 중인 모든 멤버 요청을 수락하시겠습니까?
              </h3>
              <div className="flex justify-center mt-[30px] gap-[5px]">
                <button
                  className="bg-myred text-white py-[5px] px-[15px] rounded-[10px]"
                  onClick={() => setShowModalAccAll(false)}
                  disabled={isBulkAccepting}
                >
                  아니오
                </button>
